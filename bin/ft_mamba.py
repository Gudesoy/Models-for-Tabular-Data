...
class FT_Mamba(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # mamba_config = MambaConfig(d_model=d_token,n_layers=1)
        self.layers = nn.ModuleList([])
        for layer_idx in range(n_layers):
            layer = nn.ModuleDict(
                {
                    'mamba': Mamba(d_model = d_token)
                }
            )
...
if __name__ == "__main__":
    ...
    # model.load_state_dict(torch.load("./output/jannis/ft_mamba/d1/checkpoint.pt")['model'])
