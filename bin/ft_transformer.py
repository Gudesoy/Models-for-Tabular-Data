...
#         self.W_q = nn.Linear(d, d).cuda()
#         self.W_k = nn.Linear(d, d).cuda()
#         self.W_v = nn.Linear(d, d).cuda()
#         self.W_out = nn.Linear(d, d).cuda() if n_heads > 1 else None
#         self.n_heads = n_heads
#         self.dropout = nn.Dropout(dropout).cuda() if dropout else None
...
#         if key_compression is not None:
#             assert value_compression is not None
#             key_compression = key_compression.cuda()
#             value_compression = value_compression.cuda()
#             k = key_compression(k.transpose(1, 2)).transpose(1, 2)
#             v = value_compression(v.transpose(1, 2)).transpose(1, 2)
#         else:
#             assert value_compression is None
...
#         q = self._reshape(q).to(torch.float16).cuda()
#         k = self._reshape(k).to(torch.float16).cuda()
#         v = self._reshape(v).to(torch.float16).cuda()

#         # Use flash attention
#         attention = flash_attn_func(q, k, v, dropout_p=self.dropout.p if self.dropout else 0.0)
        
#         x = (
#             attention.reshape(batch_size, self.n_heads, n_q_tokens, d_head_value)
#             .transpose(1, 2)
#             .reshape(batch_size, n_q_tokens, self.n_heads * d_head_value)
#         )
...
if __name__ == "__main__":
    ...
    # model.load_state_dict(torch.load("./output/jannis/ft_transformer/tuning/d1/checkpoint.pt")['model'])
