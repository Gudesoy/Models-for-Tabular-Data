...
class Hyena(nn.Module):
    def __init__(self, config):
        ...
        self.blocks = nn.ModuleList(get_block(config, layer_idx, flash_fft=self.flash_fft) for layer_idx in range(config.num_layers))
...
if __name__ == '__main__':
    config = dotdict(yaml.load(open("../bin/test.yml"), Loader=yaml.FullLoader))
    model = Hyena(config).to("cuda")
