...
def make_serializable(obj):
    """Recursively convert a dictionary to a JSON-serializable format."""
    if isinstance(obj, dict):
        ...
        return {make_serializable(v) for v in obj}
    elif isinstance(obj, torch.ByteTensor):
        return obj.tolist()
    elif isinstance(obj, (int, float, str, bool)) or obj is None:
        return obj
    else:
        return str(obj)  # Convert non-serializable objects to their string representation

def dump_json(x: ty.Any, path: ty.Union[Path, str], *args, **kwargs) -> None:
    serializable_x = make_serializable(x)
    Path(path).write_text(json.dumps(serializable_x, *args, **kwargs) + '\n')

def dump_stats(stats: dict, output_dir: Path, final: bool = False) -> None:
    # Ensure all data in stats is serializable
    serializable_stats = make_serializable(stats)
    dump_json(serializable_stats, output_dir / 'stats.json', indent=4)
        ...
        if json_output_path:
            try:
                ...
                try:
                    ...
                    json_data = {}
                json_data[key] = serializable_stats
