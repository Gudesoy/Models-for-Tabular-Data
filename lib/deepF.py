...
class IndexLoader:
    def __init__(
        ...
        self._device = device
        self._current_epoch = 0  # Initialize the current epoch counter
    ...
    def __iter__(self):
        indices = list(
            torch.utils.data.DataLoader(
                range(self._train_size), batch_size=self._batch_size, shuffle=self._shuffle
            )
        )
    ...
    def epoch(self):
        ...
        return torch.cat(indices).to(self._device).split(self._batch_size)
    ...
    def load_state_dict(self, state_dict):
        self._train_size = state_dict['train_size']
        self._batch_size = state_dict['batch_size']
        self._shuffle = state_dict['shuffle']
        self._device = torch.device(state_dict['device'])
